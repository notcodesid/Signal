...
export function YieldLoopCard({ link }: { link: YieldLoopLink }) {
  return (
    <div className="my-2 w-full rounded-2xl border border-amber-200 bg-amber-50 p-3 text-xs">
      <div className="mb-2 flex items-center justify-between">
        <span className="font-semibold text-amber-900">Leverage loop</span>
        <span className="rounded-full bg-amber-100 px-2 py-0.5 text-[9px] uppercase tracking-wider text-amber-700">
          deep link
        </span>
      </div>
      ...
      {link.notes && (
        <p className="mt-2 text-[10px] text-amber-800/80">{link.notes}</p>
      )}
      <p className="mt-2 text-[10px] text-gray-500">
        Native loop execution is coming. Open the protocol UI to complete the
        loop.
      </p>
      <a
        href={link.url}
        ...
        className="mt-3 inline-flex w-full items-center justify-center rounded-full bg-amber-600 px-3 py-1.5 text-[11px] font-semibold text-white hover:bg-amber-700"
      >
