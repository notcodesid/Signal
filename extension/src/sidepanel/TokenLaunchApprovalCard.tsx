...
export function TokenLaunchApprovalCard({
  ...
  return (
    <div className="my-2 w-full rounded-2xl border border-emerald-200 bg-emerald-50 p-3 text-xs">
      <div className="mb-2 flex items-center justify-between">
        <span className="font-semibold text-emerald-900">Token launch</span>
        <span className="rounded-full bg-emerald-100 px-2 py-0.5 text-[9px] uppercase tracking-wider text-emerald-700">
          {preview.cluster}
        </span>
      </div>
      ...
      <p className="mt-2 text-[10px] text-gray-500">
        Wallets show this token by mint address until Metaplex metadata is
        added.
      </p>
      ...
      {!finished && (
        <div className="mt-3 flex gap-2">
          <button
            ...
            onClick={onApprove}
            disabled={busy}
            className="flex-1 rounded-full bg-emerald-600 px-3 py-1.5 text-[11px] font-semibold text-white hover:bg-emerald-700 disabled:opacity-50"
          >
      ...
      {state === "rejected" && (
        <div className="mt-3 text-[11px] text-gray-500">Launch rejected.</div>
      )}
      {state === "error" && error && (
